  ...
  if (contactForm) {
    contactForm.addEventListener('submit', async function(e) {
      ...
      if (!emailRegex.test(email)) {
        alert('אנא הזינו כתובת דואר אלקטרוני תקינה');
        return;
      }
      ...
      if (!window.zappyContactFormLoaded) {
        try {
          ...
          const message = messageField ? messageField.value : `Name: ${name}\nEmail: ${email}\nPhone: ${phone}`;
          ...
          const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
            ...
            body: JSON.stringify({
              websiteId: 'dcdda5d9-fb87-4be1-b4b2-47a112cf7d9a',
              name: name,
              email: email,
              subject: subject,
              message: message,
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "VANGUS: אחסון אתרים בענן של גוגל קלאוד משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
